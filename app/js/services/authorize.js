...
function AuthorizationService($rootScope, $state, Principle) {
  'ngInject';
  ...
  service.authorize = function() {
    return Principle.identity()
      .then(function () {
        ...
        if ($rootScope.toState.data.roles && $rootScope.toState.data.roles.length > 0 && !Principle.isInAnyRole($rootScope.toState.data.roles)) {
          if (isAuthenticated) {
            $state.go('Site.Home');
          }
        } else {
          ...
          // Now send them to the login state so they can login
          $state.go('Login');
        }
      })
  };
